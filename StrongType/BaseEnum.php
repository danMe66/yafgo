 ...
 * Class Container_StrongType_BaseEnum
 */
class Container_StrongType_BaseEnum
{
     ...
     * @return mixed
     * @throws \ReflectionException
     * @throws container_exception_UnexpectedValueException
     */
    public function getValue($enumName)
    {
        ...
        if ($enum === false) {
            throw new container_exception_UnexpectedValueException(-1, '不存在的枚举定义');
        }
        return $enum;
    }
        ...
        if ($this->checkValue) {
            ...
            foreach ($value as $item) {
                if (!is_numeric($item) && !is_string($item)) {
                    throw new container_exception_UnexpectedValueException(-1, '错误的枚举定义，只能是数字或是字符串');
                }
            }
        }
